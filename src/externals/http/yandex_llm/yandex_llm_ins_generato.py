...
class InsGeneratorClient(YandexLLMClient):
    ...
    def __init__(self, **kwargs):
        ...
        self.search_index = kwargs.pop(
            'search_index', self.LLM_DEFAULT_SEARCH_INDEX
        )
    ...
    def _build_messages_and_schema(self, content_dict: dict, section: str):
        section_template = INS_GENERATE_PROMTS.get(
            section, INS_GENERATE_PROMTS.get('general', '')
        )
    ...
    async def _call_llm(self, payload: dict) -> dict:
        ...
        try:
            ...
            raise LLMResonseError(
                f'HTTP request to LLM failed: {exc}'
            ) from exc
        try:
        ...
        for sec in INS_GENERATE_PROMTS.keys():
            try:
                ...
                self.logger.exception(
                    'Unexpected error while generating section %s', sec
                )
                raise LLMResonseError(
                    f'Unexpected error on section {sec}: {e}'
                ) from e
        return results
